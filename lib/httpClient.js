...
class HttpClient {
	...
	start() {
		if (this.params.rpsInterval) {
			const rps = this.params.requestsPerSecond;
				...
				let f = "Summary: " + '\n'; 
				f = f + 'total time: ';
				m = (cumulative_time/1000).toFixed(2) + ' s\n';
				f = f + m + 'total requests: ' + num_rquests + '\n';
				
				var num = cumulative_time/1000;
				f = f + 'throughput: ' + (mx/num).toFixed(2) + ' req/s\n';
	...
	percentile(){
		let f = 'errors: ' + errs + '\n' + 'average: ' + (avg/mx).toFixed(2) + ' ms\n' + 'fastest: ' + fastest.toFixed(2) + ' ms\n'+ 'slowest: ' + slowest.toFixed(2) + ' ms\n' ;
					...
					f = '25%ile Latency: ' + dta[p25].toFixed(2) + ' ms\n';
					...
					f =  f +'99%ile Latency: ' + dta[p99].toFixed(2) + ' ms\n';
					

					var p999 =( 0.999*(mx-1)).toFixed();
					f = f + '99.9%ile Latency: ' + dta[p999].toFixed(2) + ' ms\n';
					fs.writeFile(fileTitle2, f, { flag: 'a' }, err => {});
	}

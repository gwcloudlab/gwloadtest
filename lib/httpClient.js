	...
	start() {
		if (this.params.rpsInterval) {
			const rps = this.params.requestsPerSecond;
				...
				var temp = this.params.rpsInterval *1000;
				var num_rquests =0;
				while (cumulative_time< temp) {
					num_rquests++;
				...
				mx = num_rquests;
				let f = "Summary: " + '\n'; 
				fs.writeFile('summary.txt', f, { flag: 'w' }, err => {});
				f = "total time: ";
				fs.writeFile('summary.txt', f, { flag: 'a' }, err => {});
		...
		return (error, result) => {
			...
			if (index==0){
				...
				if (index==mx-1){
					...
					f = '25%ile Latency: ' + dta[p25] + '\n';
					fs.writeFile('summary.txt', f, { flag: 'a' }, err => {});

					var p50 =( 0.50*(mx-1)).toFixed();
					f = '50%ile Latency: ' + dta[p50] + '\n';
					fs.writeFile('summary.txt', f, { flag: 'a' }, err => {});

					var p75 =( 0.75*(mx-1)).toFixed();
					f = '75%ile Latency: ' + dta[p75] + '\n';
					fs.writeFile('summary.txt', f, { flag: 'a' }, err => {});
			...
			if (result) {
				...
				//result.startTime = this.operation.latency.getStartTime(id);
			}
			let callback;
			if (!this.params.requestsPerSecond) {

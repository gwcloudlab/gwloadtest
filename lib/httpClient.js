...
const req_url = []; // added for gwloadtest preserving urls
var url_list = null; //added for gwloadtest client behaviour 
var total; //added for gwloadtest client behaviour 
var prev; //added for gwloadtest client behaviour 
var next = []; //added for gwloadtest client behaviour 
var cur ;
...
class HttpClient {
	...
	start() {
		if (this.params.rpsInterval) {
			const rps = this.params.requestsPerSecond;
				...
				var num_rquests =0;
				if (this.params.mode == 'closed'){ // if the loop is closed then parse previous html for urls
					this.parseHtml(prev);
					var interval = 1000 * this.randomExp(rps);
					cur += interval;
					timeStamps[num_rquests] = cur;
					num_rquests++;
					setTimeout(() => this.makeRequest(), cur); 	// sets a timer for all the requests 	
				}
	...
	parseHtml( prev ){
		...
		//console.log("string length : " + temp.length);
		...
		for (var i = 0; i<temp.length; i++){
			if (i < temp.indexOf('</a>', i)){ // parse hyperlinks
				...
				if ( temp.indexOf('href="', i)>0){
					...
					//console.log("next: " + temp.substring(start+6, end));
	...
	makeRequest() {
		...
		if (typeof this.params.requestGenerator == 'function') {
			...
			else if (this.params.mode == 'closed'){
				//console.log("closed");
				//console.log(next);
	...
	getRequestFinisher(id, timestamp) {
		return (error, result) => {
			...
			if (result) {
				...
				this.parseHtml(prev);
				var intr = this.params.rpsInterval *1000;
				if (this.params.mode == 'closed' && cur<intr){ // if the loop is closed then parse previous html for urls
					this.parseHtml(prev);
					var interval = 1000 * this.randomExp(rps);
					cur += interval;
					timeStamps[num_rquests] = cur;
